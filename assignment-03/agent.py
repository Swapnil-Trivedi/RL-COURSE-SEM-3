    ...
    def update(self):
        ...
        with torch.no_grad():
            if self.use_double_dqn:
                # TODO: Implement Double DQN target computation
                # --- Double DQN Logic ---
                ...
                # TODO: Implement Standard DQN target computation
                # --- Standard DQN Logic ---
                # Use target net for both selection and evaluation: max_a' Q_target(s', a')
                next_q = self.target_net(next_states).max(1)[0]
                pass

            # TODO: Calculate final target Q-values using Bellman equation
        ...
        if self.use_prioritized_replay and idxs is not None:
            with torch.no_grad():
                # TODO: Recompute TD errors using the updated policy network.
                ...
                new_current_q = self.policy_net(states).gather(1, actions.unsqueeze(1)).squeeze(1)
                td_errors = torch.abs(target_q - new_current_q)
